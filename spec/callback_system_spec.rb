...
RSpec.describe "OpenRouter Callback System" do
  ...
  describe "callback registration" do
    it "allows registering callbacks for valid events" do
      ...
      expect {
        client.on(:after_response) { |response| callback_called = true }
      }.not_to raise_error
    ...
    it "raises error for invalid events" do
      ...
      }.to raise_error(ArgumentError, /Invalid event: invalid_event/)
    end
  ...
  describe "callback clearing" do
    before do
      client.on(:after_response) { |response| "callback1" }
      client.on(:after_response) { |response| "callback2" }
      client.on(:before_request) { |params| "callback3" }
    end
  ...
  describe "callback triggering" do
    ...
    it "handles callback errors gracefully" do
      client.on(:after_response) { |response| raise "callback error" }
  ...
  describe "callback integration with API calls" do
    ...
    it "doesn't trigger on_tool_call when no tool calls present" do
      ...
      client.on(:on_tool_call) { |tool_calls| tool_call_triggered = true }
