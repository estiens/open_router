...
RSpec.describe OpenRouter::StreamingClient do
  ...
  describe "#on_stream" do
    it "registers streaming callbacks for valid events" do
      ...
      expect {
        client.on_stream(:on_chunk) { |chunk| callback_called = true }
      }.not_to raise_error
    ...
    it "raises error for invalid streaming events" do
      ...
      }.to raise_error(ArgumentError, /Invalid streaming event: invalid_event/)
    end
  ...
  describe "#stream_complete" do
    ...
    before do
      ...
      allow(client).to receive(:complete) do |*args, **kwargs, &block|
        if kwargs[:stream]
          mock_chunks.each { |chunk| kwargs[:stream].call(chunk) }
        end
        nil
      end
    end
  ...
  describe "#stream" do
    before do
      ...
      allow(client).to receive(:stream_complete) do |*args, **kwargs, &block|
        mock_chunks = [
        ...
        mock_chunks.each { |chunk| block.call(chunk) if block }
        nil
      end
    end
  ...
  describe "tool call streaming" do
    ...
    before do
      allow(client).to receive(:complete) do |*args, **kwargs|
        if kwargs[:stream]
          tool_call_chunks.each { |chunk| kwargs[:stream].call(chunk) }
        end
        nil
      end
    end
